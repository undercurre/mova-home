...
/// 检查配网条件
mixin PairNetCheckMixin on BasePageState, CommonDialog {
  /// 校验条件
  ...
  bool checkNetworkConnectAP() {
    final currentWifiName =
        ref.watch(routerPasswordStateNotifierProvider).routerWifiName ?? '';
  ...
  /// 检测合法的SSID
  static bool isValidWifiName(String name) {
    List<String> array = [
      'dreame_dock_',
      'dreame-vacuum-',
      'dreame-hold-',
  ...
  bool checkNetworkFrequencyBand() {
    final frequency = ref.watch(routerPasswordStateNotifierProvider).frequency;
    ...
    if (is24G) {
      // LogModule().eventReport(
      //     ModuleCode.PairNetNew.code, PairNetEventCode.Is5GHz.code,
      //     int1: 0,
      //     str1: GenerateSessionID().currentSessionID(),
      //     str2: '$frequency');
      return true;
  ...
  bool confirmNoPassword() {
    final routerWifipwd =
        ref.watch(routerPasswordStateNotifierProvider).routerWifipwd;
        ...
        builder: (_) {
          return NetBrandErrorDialog(
            ...
            guideCallback: () async {
              ...
              final url =
                  '$webHost/connectNetwork/connectGuide.html?lang=$lang&tenantId=$tenantId&themeMode=$themePath';
