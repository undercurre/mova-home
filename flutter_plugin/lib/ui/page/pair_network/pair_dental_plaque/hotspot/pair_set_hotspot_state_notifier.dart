...
import 'package:flutter_plugin/ui/page/pair_network/pair_dental_plaque/hotspot/pair_set_hotspot_uistate.dart';
...
part 'pair_set_hotspot_state_notifier.g.dart';

@riverpod
class PairSetHotspotStateNotifier extends _$PairSetHotspotStateNotifier
    with PairNetWifiInfoMixin {
  @override
  PairSetHotspotUiState build() {
    return const PairSetHotspotUiState();
  }
  ...
  void enableBtn() {
    ...
    /// 如果WiFi名称和密码有一个是空的 则按不可点击
    if (routerWifiName?.isEmpty == true || routerWifipwd.isEmpty == true) {
      state = state.copyWith(enableBtn: false);
      return;
    }
    /// 如果Wifi名称不为空，密码不为空，则需要检测WiFi名称和密码长度
    if (routerWifiName?.isNotEmpty == true &&
        routerWifipwd.isNotEmpty == true) {
