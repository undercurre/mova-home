...
import 'package:flutter_plugin/ui/page/message/system/system_message_list_notifier.dart';
...
class SystemMessageListPage extends BasePage {
  static const String routePath = '/message_system';

  const SystemMessageListPage({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() {
    return SystemMessageListState();
  }
}

class SystemMessageListState extends BasePageState
  ...
  void addObserver() {
    ref.listen(
        systemMessageListNotifierProvider.select((value) => value.uiEvent),
        (previous, next) {
      if (next != null) {
        responseFor(next);
      }
  ...
  void initData() {
    ref
        .read(systemMessageListNotifierProvider.notifier)
        .refreshSystemMessage(forceRead: true);
  ...
  PreferredSizeWidget? buildNavBar(
    ...
    List<CommonMsgRecord> msgList =
        ref.watch(systemMessageListNotifierProvider).systemMessageList;
    return NavBar(
      backHidden: false,
      itemAction: (tag) {
            ...
            child: SlidableAutoCloseBehavior(
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  if (msgList.isEmpty) {
                    return Container(
                      margin: const EdgeInsets.only(top: 100).withRTL(context),
                      width: double.infinity,
                      child: Column(
          ...
          children: [
            Container(
                ...
                child: Center(
                  ...
                  ref
                      .read(systemMessageListNotifierProvider.notifier)
                      .deleteSystemMessageByMsgId(message.id ?? '');
                })))
          ],
        ),
