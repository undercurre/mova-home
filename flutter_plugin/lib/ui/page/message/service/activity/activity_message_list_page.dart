...
import 'activity_message_list_notifier.dart';

class ActivityListPage extends BasePage {
  const ActivityListPage({super.key});

  @override
  ActivityListPageState createState() {
    return ActivityListPageState();
  }
}

class ActivityListPageState extends BasePageState
  ...
  void addObserver() {
    ref.listen(
        activityMessageListNotifierProvider.select((value) => value.uiEvent),
        (previous, next) {
      if (next != null) {
        responseFor(next);
      }
  ...
  Widget buildBody(
    ...
    List<CommonMsgRecord> msgList =
        ref.watch(activityMessageListNotifierProvider).activityMessageList;
    return Container(
    ...
    return GestureDetector(
        ...
        child: Container(
            ...
            child: Column(
              children: [
                Visibility(
                    visible: message.imgUrl?.isNotEmpty == true,
                    child: ClipRRect(
                      borderRadius:
                          const BorderRadius.vertical(top: Radius.circular(0)),
                      child: (message.imgUrl ?? '').isEmpty == true
                          ? Container()
                          : CachedNetworkImage(
                              fit: BoxFit.fill,
