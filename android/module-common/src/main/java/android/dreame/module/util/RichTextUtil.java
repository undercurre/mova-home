...
public class RichTextUtil {
    ...
    public static CharSequence getColorString(String originText, String keyword, int color,
        ...
        String keywordEscape = characterEscape(keyword);
        SpannableString s = new SpannableString(originText);
        Pattern p = Pattern.compile(keywordEscape);
        Matcher m = p.matcher(originText);

        while (m.find()) {
            int start = m.start();
            int end = m.end();
            final String substring = originText.substring(start, end);
            if (keyword.equalsIgnoreCase(substring)) {
    ...
    public static CharSequence getColorString(String originText, List<String> keywords,
        ...
        for (int i = 0; i < keywords.size(); i++) {
            String characterEscape = characterEscape(keywords.get(i));
            keywords.set(i, characterEscape);
        }
        SpannableString s = new SpannableString(originText);
        ...
        for (int i = 0; i < keywords.size(); i++) {
            ...
            while (m.find()) {
                int start = m.start();
                int end = m.end();
                String substring = originText.substring(start, end);
    ...
    public static String characterEscape(String characters) {
        StringBuilder stringBuffer = new StringBuilder();
        char[] chars = characters.toCharArray();
        for (char character : chars) {
            stringBuffer.append(characterEscape(character));
        }
     ...
     * 字符转义
     */
    public static String characterEscape(char character) {
        switch (character) {
            case '$':
                return "\\$";
            case '(':
