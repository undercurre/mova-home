...
public class LetterSideBarView extends View {
    ...
    public LetterSideBarView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取属性
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LetterSideBarView);
        if (typedArray != null) {
            mColor = typedArray.getColor(R.styleable.LetterSideBarView_android_textColor, mColor);
            mHighlightColor = typedArray.getColor(R.styleable.LetterSideBarView_highlightColor, mHighlightColor);
            mSize = typedArray.getDimensionPixelSize(R.styleable.LetterSideBarView_android_textSize, (int) mSize);
            mLetterSpace = typedArray.getDimensionPixelSize(R.styleable.LetterSideBarView_letterSpace, 0);
            mHighlightBackground = typedArray.getColor(R.styleable.LetterSideBarView_highlightBackground, mHighlightBackground);
            typedArray.recycle();
        }
        mPaint = new Paint();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mLetters.length; i++) {
            ...
            if (mLetterIndex == i) {
                ...
                canvas.drawCircle( getWidth() / 2, dy, base + mLetterSpace / 2, mPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //手指滑动
                if (onLetterChangeListener != null && !TextUtils.isEmpty(letter) && !TextUtils.equals(lastSelectLetter,letter)) {
                    onLetterChangeListener.onLetterChanged(letter);
                }
                invalidate();
                lastSelectLetter = letter;
                break;
