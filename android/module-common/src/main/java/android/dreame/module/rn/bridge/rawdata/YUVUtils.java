...
public class YUVUtils {
    ...
    public static byte[] i420ToJpg(int width, int height, int rotation, int bufferLength, byte[] buffer, int yStride, int uStride, int vStride) {
        byte[] NV21 = new byte[bufferLength];
        swapYU12toYUV420SP(buffer, NV21, width, height, yStride, uStride, vStride);
    ...
    public static byte[] I420_to_NV21(byte[] src, int width, int height) {
        byte[] dst = new byte[src.length];
        System.arraycopy(src, 0, dst, 0, src.length);
        for (int i = 0; i < width * height / 4; i++) {
            dst[width * height + 2 * i + 1] = src[width * height + i];//U
            dst[width * height + 2 * i] = src[width * height + width * height / 4 + i];//V
        }
        return dst;
    }

    public static byte[] I420_to_NV12(byte[] src, int width, int height) {
        byte[] dst = new byte[src.length];
        System.arraycopy(src, 0, dst, 0, src.length);
        for (int i = 0; i < width * height / 4; i++) {
            dst[width * height + 2 * i + 1] = src[width * height + width * height / 4 + i];//V
            dst[width * height + 2 * i] = src[width * height + i];//U
        }
        return dst;
    }
}
