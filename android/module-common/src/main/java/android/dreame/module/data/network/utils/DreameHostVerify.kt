...
object DreameHostVerify : HostnameVerifier {

    private val TAG: String = "DreameHostVerify"
    ...
    override fun verify(host: String, session: SSLSession): Boolean {
        ...
        if (host.endsWith("mova-tech.com")) {
            return try {
    ...
    private fun verify(host: String, certificate: X509Certificate): Boolean {
        val subjectCN = getSubjectDN(certificate)
        return when {
            host.canParseAsIpAddress() -> verifyIpAddress(host, certificate, subjectCN)
            else -> verifyHostname(host, certificate, subjectCN)
        }
    }
    ...
    private fun verifyIpAddress(
        ...
        return getSubjectAltNames(certificate, ALT_IPA_NAME).any {
            Log.d(
                TAG,
                "verifyIpAddress: $canonicalIpAddress, ALT_IPA_NAME: $it, subjectCN:$subjectCN"
            )
            canonicalIpAddress == it.toCanonicalHost() && it == subjectCN
        }
    }
    ...
    private fun verifyHostname(
        ...
        return getSubjectAltNames(certificate, ALT_DNS_NAME).any {
            Log.d(TAG, "verifyHostname: $host, ALT_DNS_NAME: $it, subjectCN:$subjectCN")
            verifyHostname(host, it) && it == subjectCN
        }
    }
    ...
    /**
     * 获取证书共用名称 CN(Common Name)
     */
    private fun getSubjectDN(certificate: X509Certificate): String {
