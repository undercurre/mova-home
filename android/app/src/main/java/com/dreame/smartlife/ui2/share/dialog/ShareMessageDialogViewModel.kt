...
class ShareMessageDialogViewModel :
    BaseViewModel<ShareMessageDialogUiState, ShareMessageDialogUiEvent>() {

    private val messageRepo: MessageRepository =
        MessageRepository(MessageRemoteDataSource())

    override fun createInitialState(): ShareMessageDialogUiState {
        return ShareMessageDialogUiState(
            false
        )
    }
    ...
    private suspend fun ackResult(it: Result<Nothing>) {
        if (it is Result.Success) {
            _uiEvents.send(ShareMessageDialogUiEvent.ShowToast(getString(R.string.operate_success)))
        } else if (it is Result.Error) {
            when (it.exception.code) {
                60000, 60400, 60900, 60901, 40040 -> {
                    _uiEvents.send(ShareMessageDialogUiEvent.ShowToast(getString(R.string.share_msg_already_invalid)))
                }

                else -> {
                    _uiEvents.send(ShareMessageDialogUiEvent.ShowToast(getString(R.string.toast_server_error)))
                }
            }
        }
        _uiEvents.send(ShareMessageDialogUiEvent.DismissDialog)
    }
