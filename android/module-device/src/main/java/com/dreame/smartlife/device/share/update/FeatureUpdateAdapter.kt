...
class FeatureUpdateAdapter(
    val onShareStatusChange: ((key: String?, status: Boolean) -> Unit)? = null,
    val onImageClick: ((imageUrl: String?, content: String?) -> Unit)? = null
) :
    ...
    override fun convert(holder: BaseViewHolder, item: DeviceFeatureShareRes) {
        ...
        val cbStatus = holder.getView<CheckBox>(R.id.cb_status)
        cbStatus.setOnCheckedChangeListener(null)
        cbStatus.isChecked = item.open
        cbStatus.setOnCheckedChangeListener { _, checked ->
            item.open = checked
            onShareStatusChange?.invoke(item.permitKey, checked)
        }
        ...
        val imageSpan = ImageSpan(drawable, ALIGN_BASELINE)
